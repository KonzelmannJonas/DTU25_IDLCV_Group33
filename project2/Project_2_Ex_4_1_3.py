...
def plot_training_curves(train_losses, val_losses, train_accs, val_accs):
    ...
    plt.tight_layout()
    plt.savefig("early_fusion.png")   # saves plot to file
    plt.close()
...
class EarlyFusionModel(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        self.cnn = FrameCNN(num_classes)
        # Modify first conv to accept 6 input channels (RGB + Gray)
        self.cnn.convolutional[0] = nn.Conv2d(6, 8, kernel_size=3, padding=1)
    
    def forward(self, frames_rgb, frames_gray):
        # Concatenate RGB and grayscale along the channel dimension
        # frames_rgb and frames_gray: [B, N, 3, H, W]
        B, N, C, H, W = frames_rgb.shape
        fused_frames = torch.cat([frames_rgb, frames_gray], dim=2)  # → [B, N, 6, H, W]

        logits_list = []
        for i in range(N):
            logits_i = self.cnn(fused_frames[:, i])  # process each frame
            logits_list.append(logits_i.unsqueeze(1))
        
        # Average predictions over all frames
        logits = torch.cat(logits_list, dim=1).mean(dim=1)
        return logits
