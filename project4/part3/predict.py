...
Usage:
    python part2/predict.py \
        ...
        --checkpoint out/checkpoints/best_model.pth \
        --splits splits.json \
        --conf-threshold 0.5 \
        --nms-threshold 0.3 \
...
def compute_average_precision(
    det_dict: Dict[str, List[Tuple[Tuple[int, int, int, int], float]]],
    gt_dict: Dict[str, List[Tuple[int, int, int, int]]],
    iou_threshold: float = 0.5,
) -> float:
    """
    Compute Average Precision (AP) over the whole dataset.

    det_dict: image_id -> list of (bbox, score)
    gt_dict:  image_id -> list of gt boxes
    """
    # Flatten all detections: (img_id, bbox, score)
    all_dets = []
    for img_id, dets in det_dict.items():
        for bbox, score in dets:
            all_dets.append((img_id, bbox, float(score)))
    ...
    # Track which GT boxes have been matched
    gt_used = {img_id: [False] * len(boxes) for img_id, boxes in gt_dict.items()}

    tps = []
    fps = []

    for img_id, det_box, score in all_dets:
        gt_boxes = gt_dict.get(img_id, [])
        best_iou = 0.0
        best_idx = -1

        # Find best matching unused GT box
...
def main():
    ...
    # For AP computation over the whole test set
    det_dict: Dict[str, List[Tuple[Tuple[int, int, int, int], float]]] = {}
    gt_dict: Dict[str, List[Tuple[int, int, int, int]]] = {}
    ...
    for stem in test_stems:
        ...
        # Store detections and ground truth for AP computation
        det_dict[stem] = detections
        if len(ground_truth) > 0:
            gt_dict[stem] = ground_truth
    ...
    if overall_metrics['TP'] + overall_metrics['FP'] > 0:
        ...
        # compute AP over whole test set

        ap = None
        if len(gt_dict) > 0:
            ap = compute_average_precision(det_dict, gt_dict, iou_threshold=args.iou_threshold)
        ...
        if ap is not None:
            print(f"Average Precision (AP) @ IoU={args.iou_threshold:.2f}: {ap:.4f}")
