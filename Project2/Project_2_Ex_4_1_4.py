...
def plot_training_curves(train_losses, val_losses, train_accs, val_accs):
    ...
    plt.tight_layout()
    plt.savefig("3D_CNN.png")   # saves plot to file
    plt.close()
...
class FrameVideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.stack_frames:
            frames = torch.stack(frames).permute(1, 0, 2, 3) # from [T, C, H, W] to [C, T, H, W]
...
class CNN3D(nn.Module):
    def __init__(self, num_classes):
        ...
        self.convolutional = nn.Sequential(
            nn.Conv3d(3, 8, kernel_size=(3, 3, 3), padding=1),  # 3 input channels; padding 1 to get full image
            nn.BatchNorm3d(8),
            nn.ReLU(),
            nn.Conv3d(8, 8, kernel_size=(3, 3, 3), padding=1),
            nn.BatchNorm3d(8),
            nn.ReLU(),
            nn.MaxPool3d((1, 2, 2)),  # 128×128 to 64×64
            ...
            nn.BatchNorm3d(16),
            nn.ReLU(),
            nn.MaxPool3d((2, 2, 2)),   # 64×64 to 32×32
