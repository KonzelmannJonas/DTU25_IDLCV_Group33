...
def point_level_loss(logits, mask, pos_xy, neg_xy, alpha=0.8):
    """
    Improved point-level loss with regularization
    
    Args:
        logits: Model predictions
        mask: Ground truth mask
        pos_xy: Positive click coordinates
        neg_xy: Negative click coordinates  
        alpha: Weight for point supervision vs regularization (0.8 = 80% point, 20% reg)
    """
    ...
    if len(losses) > 0:
        point_loss = torch.stack(losses).sum() / total_points
    else:
        point_loss = torch.tensor(0.0, device=logits.device, requires_grad=True)
    
    # Add regularization: encourage smoothness and consistency
    # Simple L2 regularization on predictions
    smoothness_loss = torch.mean(torch.abs(logits[:, :, :-1, :] - logits[:, :, 1:, :])) + \
                     torch.mean(torch.abs(logits[:, :, :, :-1] - logits[:, :, :, 1:]))
    
    # Combine losses
    total_loss = alpha * point_loss + (1 - alpha) * smoothness_loss
...
SAMPLING_METHOD = "boundary"  # Change this to test different sampling methods

def analyze_click_distribution(dataset, num_samples=10):
    """Analyze the spatial distribution of clicks to ensure they're diverse"""
    import numpy as np
    
    print(f"\n=== Analyzing {dataset.clicks_pos} boundary clicks distribution ===")
    
    all_pos_clicks = []
    all_neg_clicks = []
    
    for i in range(min(num_samples, len(dataset))):
        _, _, pos_xy, neg_xy = dataset[i]
        
        # Filter valid clicks
        valid_pos = [(x, y) for x, y in pos_xy if x >= 0 and y >= 0]
        valid_neg = [(x, y) for x, y in neg_xy if x >= 0 and y >= 0]
        
        all_pos_clicks.extend(valid_pos)
        all_neg_clicks.extend(valid_neg)
    
    print(f"Average positive clicks per image: {len(all_pos_clicks) / num_samples:.2f}")
    print(f"Average negative clicks per image: {len(all_neg_clicks) / num_samples:.2f}")
    
    if all_pos_clicks:
        pos_x = [x for x, y in all_pos_clicks]
        pos_y = [y for x, y in all_pos_clicks]
        print(f"Positive clicks - X range: [{min(pos_x)}, {max(pos_x)}], Y range: [{min(pos_y)}, {max(pos_y)}]")
        print(f"Positive clicks - X std: {np.std(pos_x):.2f}, Y std: {np.std(pos_y):.2f}")
    
    if all_neg_clicks:
        neg_x = [x for x, y in all_neg_clicks]
        neg_y = [y for x, y in all_neg_clicks]
        print(f"Negative clicks - X range: [{min(neg_x)}, {max(neg_x)}], Y range: [{min(neg_y)}, {max(neg_y)}]")
        print(f"Negative clicks - X std: {np.std(neg_x):.2f}, Y std: {np.std(neg_y):.2f}")
    
    return len(all_pos_clicks), len(all_neg_clicks)

# Load UNET
results_by_clicks = {}

for i in [5, 10, 15, 20, 25, 30]:
    print(f"\n=== Training with {i} boundary clicks ===")
    ...
    # Analyze click distribution for debugging
    analyze_click_distribution(train_ds, num_samples=5)

    best_val_dice = 0.0
    best_metrics = None
    
    # click annotations, training loop - increased epochs
    ...
    results_by_clicks[i] = metrics
    print(f"Final test metrics for {i} clicks: {metrics}")

# Print summary of all results
print("\n=== SUMMARY OF RESULTS ===")
for clicks, metrics in results_by_clicks.items():
    print(f"{clicks} clicks: Dice={metrics['dice']:.4f}, IoU={metrics['iou']:.4f}, Acc={metrics['acc']:.4f}")
    
# Plot results
import matplotlib.pyplot as plt
clicks_list = list(results_by_clicks.keys())
dice_scores = [results_by_clicks[k]['dice'] for k in clicks_list]
iou_scores = [results_by_clicks[k]['iou'] for k in clicks_list]

plt.figure(figsize=(10, 6))
plt.subplot(1, 2, 1)
plt.plot(clicks_list, dice_scores, 'bo-', label='Dice Score')
plt.xlabel('Number of Boundary Clicks')
plt.ylabel('Dice Score')
plt.title('Dice Score vs Number of Clicks')
plt.grid(True)
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(clicks_list, iou_scores, 'ro-', label='IoU Score')
plt.xlabel('Number of Boundary Clicks')
plt.ylabel('IoU Score')
plt.title('IoU Score vs Number of Clicks')
plt.grid(True)
plt.legend()

plt.tight_layout()
plt.savefig('performance_vs_clicks.png', dpi=150)
plt.show()
...
# # saves the plain image and the mask on the right
# fig, axes = plt.subplots(1, 2, figsize=(8, 4))
# x, y, _, _ = train_ds[0]  # use the first sample (or change index)
...
# plt.tight_layout()
# plt.savefig(f"/zhome/9c/f/221532/Project3/part2/Ablation_results/image_and_mask.png", dpi=150)
# plt.close()
...
# # saves images only with clicks for visual inspection
...
#     # Plot only the image (with points, if already overlaid in xi)
#     fig, ax = plt.subplots()
#     ax.imshow(img_np, cmap=img_cmap)
#     ax.axis("off")
...
#     plt.tight_layout()
#     plt.savefig(
#         f"/zhome/9c/f/221532/Project3/part2/Ablation_results/clicks_image_{SAMPLING_METHOD}_{i+1}.png",
#         dpi=150,
#         bbox_inches="tight",
#         pad_inches=0
#     )
